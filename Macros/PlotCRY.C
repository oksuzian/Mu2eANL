...
void Compare_two_plots(std::string variable, const char* cut, int cut_version=0){
  ...
  tree_neg->Draw(Form("%s>>hCRY", variable.c_str()),"", "hist");
  tree_pos->Draw(Form("%s>>hDBY", variable.c_str()),"", "same hist");
  ...
  // double maxNeg = hCRY->GetXaxis()->GetBinCenter(hCRY->GetMaximumBin());
  // double maxPos = hDBY->GetXaxis()->GetBinCenter(hDBY->GetMaximumBin());
  ...
  c1->SaveAs(Form("plots/var%s_%d.pdf", plot_name.c_str(), cut_version));
  c1->SetLogy();
  c1->SaveAs(Form("plots/var%s_%d_log.pdf", plot_name.c_str(), cut_version));
  hCRY->Delete();
  hDBY->Delete();
...
void Compare_two_graphs(const char* variable, const char* name, const char* cut1, const char* cut2,int cut_version=0){
  ...
  TCanvas *c1 = new TCanvas("c1","c1",1800,600);
  tree_neg->Draw(variable, cut2);
  TGraph *gr2 = new TGraph(tree_neg->GetSelectedRows(), tree_neg->GetV2(), tree_neg->GetV1());
  tree_neg->Draw(variable, cut1);
  TGraph *gr1 = new TGraph(tree_neg->GetSelectedRows(), tree_neg->GetV2(), tree_neg->GetV1());
...
void PlotCRY(){
  ...
  //  TFile file1("/mu2e/data/users/oksuzian/nts.oksuzian.trkanadigi.trkana.root");
  // TFile file1("/mu2e/data/users/oksuzian/nts.oksuzian.trkanadigi.trkana_nocut.root");
  // TFile file2("/mu2e/data/users/oksuzian/nts.oksuzian.trkanadigi.trkana_nocut_fullsample.root");
  //  TFile file2("/mu2e/data/users/oksuzian/nts.oksuzian.trkanadigi.trkana_softcalo.root");
  //  TFile file1("/mu2e/data/users/oksuzian/nts.oksuzian.trkanadigi.trkana_calo.root");
  //  TFile file2("/mu2e/data/users/oksuzian/nts.oksuzian.trkanadigi.trkana_nocutsv2.root");
  ...
  // TFile file2("/mu2e/data/users/oksuzian/nts.oksuzian.trkana.unmix.cry2.reco.root");
  tree_neg = (TTree*) file2.Get("TrkAnaNeg/trkana");
  tree_pos = (TTree*) file1.Get("TrkAnaNeg/trkana");
  //  tree_pos = (TTree*) file1.Get("TrkAnaPos/trkana");

  tree_pos->SetLineColor(kRed);
  tree_pos->SetMarkerColor(kRed);
  tree_neg->SetMarkerStyle(6);
  tree_neg->SetMarkerColor(kBlue);
  tree_neg->SetLineColor(kBlue);
  ...
  tree_pos->Draw("demcgen.pdg", "de.mom>0 && abs(demcgen.pdg)!=13");
  c1->SaveAs("plots/PrimaryPDG_pos.pdf");

  tree_neg->Draw("demc.pdg", signal_all.c_str(), "hist TEXT");
  c1->SaveAs("plots/PDG_neg.pdf");
  tree_neg->Draw("ncrv", signal_all.c_str(), "hist TEXT");
  c1->SaveAs("plots/ncrv_neg.pdf");
  // tree_pos->Draw("demc.pdg", signal_all.c_str(), "hist TEXT");
  // c1->SaveAs("plots/PDG_pos.pdf");

  ((TTreePlayer*)(tree_neg->GetPlayer()))->SetScanRedirect(true); 
  ((TTreePlayer*)(tree_neg->GetPlayer()))->SetScanFileName("signal_events.txt"); 
  tree_neg->Scan("de.mom:demc.pdg:evtinfo.eventid:evtinfo.subrunid:@crvinfo.size():demcgen.pdg", signal_all.c_str());

  // tree_neg2->Draw("crvinfomc._x[0]:crvinfomc._z[0]", "de.mom>100", "same");
