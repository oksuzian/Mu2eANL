...
void makeStandardizedPlots(string treePath, bool neg, bool makeCuts, bool useMomCut = true, bool onlyScan = false)
{
  ...
  string timing_cut = "de.t0>700 && de.t0<1700"; // This is the standard window
  //  string timing_cut = "de.t0>0 && de.t0<16000"; //Use 1600 to avoid edge effects on DBY plots
  ...
  if (false) //////////////////////////////////////////////////// Change this if you want the below scans to run
  ...
  else 
    {
      ...
      tree->Scan("evtinfo.subrunid:evtinfo.eventid:demc.pdg",cuts + "abs(demc.pdg)>211","");
      cout << "Events which were not produced by a muon that did not produce coincidences in the CRV:" << endl;
      tree->Scan("evtinfo.subrunid:evtinfo.eventid:demcgen.pdg:demc.pdg:deent.mom:de.t0",cuts +  "abs(demcgen.pdg)!=13" + "@crvinfo.size()<1","");
      cout << "\n" << endl;  
  ...
  */
  ...
  //  tree->Draw("deent.mom>>+h_deent_mom",*(cuts && "*de.t0", "goff");
